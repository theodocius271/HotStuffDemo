...
type reqSetImpl struct {
	lock  sync.Mutex
	order list.List
	set   map[string]*list.Element
}

func NewReqSet() *reqSetImpl {
	c := &reqSetImpl{
...
func (c *reqSetImpl) Add(cmds ...string) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *reqSetImpl) Remove(cmds ...string) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *reqSetImpl) GetFirst(n int) []string {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *reqSetImpl) IsProposed(cmd string) bool {
	c.lock.Lock()
	defer c.lock.Unlock()
	if e, ok := c.set[cmd]; ok {
...
func (c *reqSetImpl) MarkProposed(cmds ...string) {
	c.lock.Lock()
	defer c.lock.Unlock()
	for _, cmd := range cmds {
		if e, ok := c.set[cmd]; ok {
...
func (c *reqSetImpl) UnMark(cmds ...string) {
	c.lock.Lock()
	defer c.lock.Unlock()
	for _, cmd := range cmds {
		if e, ok := c.set[cmd]; ok {
